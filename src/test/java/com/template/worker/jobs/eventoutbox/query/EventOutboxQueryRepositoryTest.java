package com.template.worker.jobs.eventoutbox.query;
...
class EventOutboxQueryRepositoryTest {

    private JdbcTemplate jdbcTemplate;
    private EventOutboxQueryRepository repository;

    @BeforeEach
    void setUp() {
        DataSource dataSource = createDataSource();
        jdbcTemplate = new JdbcTemplate(dataSource);
        repository = new EventOutboxQueryRepository(new NamedParameterJdbcTemplate(dataSource));
    ...
    void pollPublishableRows_readsPublishableRowsOnly() {
        ...
        assertThat(rows)
                .extracting(EventOutboxRow::eventId)
    ...
    void markMethods_updateState() {
        ...
        assertThat(
                ...
                .isEqualTo(EventOutboxStatus.PUBLISH_PENDING.name());
        assertThat(
                        jdbcTemplate.queryForObject(
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT status FROM usage_event_outbox WHERE id = 1", String.class))
                .isEqualTo(EventOutboxStatus.FAILED.name());
        ...
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT status FROM usage_event_outbox WHERE id = 1", String.class))
                .isEqualTo(EventOutboxStatus.SENT.name());
    }
    ...
    void countAndAgeQueries_returnOperationalValues() {
        ...
        assertThat(repository.countByStatus(EventOutboxStatus.PUBLISH_PENDING)).isEqualTo(2);
        assertThat(repository.countByStatus(EventOutboxStatus.FAILED)).isEqualTo(1);
        assertThat(repository.countByStatus(EventOutboxStatus.SENT)).isEqualTo(1);
